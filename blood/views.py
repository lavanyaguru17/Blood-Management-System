...
def get_requester_type(request):
    if hasattr(request, 'request_by_patient'):
        return 'patient'
    elif hasattr(request, 'request_by_donor'):
        return 'donor'
    else:
        return None
...
def update_approve_status_view(request,pk):
    ...
    if stock.unit > unit:
        ...
        stock.save()
        req.status="Approved"
        requester_type = get_requester_type(req)
        if requester_type:
            # Send email to the requester
            if req.request_by_patient:
                recipient_email = User.objects.get(id=req.request_by_patient.user_id).email
            elif req.request_by_donor:
                recipient_email = User.objects.get(id=req.request_by_donor.user_id).email
...
def approve_donation_view(request, pk):
    ...
    user = User.objects.get(id=donation.donor.user_id)
    send_mail('Welcome to our blood bank', 'Thank you for donating blood! Your donation has been approved and added to our stock.',
              'vsabari146@gmail.com',  # Sender's email address
              [user.email],  # Recipient's email address (donor's email)
              fail_silently=False,
              )
    return HttpResponseRedirect('/admin-donation')
...
def reject_donation_view(request, pk):
    ...
    user = User.objects.get(id=donation.donor.user_id)
    send_mail('Blood Donation Rejection', 
              'Thank you for your willingness to donate blood. Unfortunately, your donation has been rejected. Please try again in the future.',
              'vsabari146@gmail.com',  # Sender's email address
              [user.email],  # Recipient's email address (donor's email)
              fail_silently=False,
              )
...
def logout_view(request):
    logout(request)
    return render(request, 'blood/logout.html')  # Assuming 'home_view' is the name of your home view
