...
def patient_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and patientForm.is_valid():
            ...
            #Send email notification to the user who signed up
            send_mail('Welcome to our blood bank','Thank you for signing up! You are now registered as a Patient.','vsabari146@gmail.com',
                      #Sender's email address
                      [user.email],
                      #Recipient's email address(user who signed up))
                      fail_silently=False,
            )
